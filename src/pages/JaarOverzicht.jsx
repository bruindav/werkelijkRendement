...
import { Building2, Plus, Trash2, ArrowRight, X, Check } from 'lucide-react';
  ...
  return (
    <div className="bg-slate-900 border border-slate-700 rounded-xl p-4 flex gap-3 items-center">
      <input
        ...
        placeholder="Naam bank / broker (bijv. ING, DeGiro, DEGIRO...)"
...
export default function JaarOverzicht() {
  ...
  const { user } = useApp();
  const { banken, loading, voegBankToe, verwijderBank } = useBanken(user?.uid, year);
  const [toonForm, setToonForm] = useState(false);

  const handleSave = async (data) => {
    await voegBankToe(data);
    setToonForm(false);
  };
  ...
  return (
    <div>
      ...
      {toonForm && (
        <div className="mb-4">
          <BankForm onSave={handleSave} onCancel={() => setToonForm(false)} />
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-slate-800/30 border border-slate-700 rounded-2xl">
          ...
          <p className="text-slate-500 text-sm mt-1">Voeg een bank of broker toe om te beginnen</p>
          <button
            onClick={() => setToonForm(true)}
        ...
        <div className="space-y-3">
          {banken.map(bank => (
            <div key={bank.id} className="bg-slate-800/40 border border-slate-700 rounded-2xl p-5 flex items-center justify-between group">
              <Link
                to={`/jaar/${year}/bank/${bank.id}`}
